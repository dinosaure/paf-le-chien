...
module Make (Flow : Mirage_flow.S) = struct
  ...
  let recv flow ~report_error ~read ~read_eof =
    ...
    | (Error _ | Ok `Eof) as v ->
        ...
        | Ok _ ->
            ()
            (* TODO(dinosaure): @ansiwen report an error in this case for
               [ocaml-h2]. *)
    ...
    let rec rd_fiber () =
      ...
      let rec go () =
        ...
        | `Read ->
            ...
            Easy_flow.recv flow ~report_error ~read:(Runtime.read connection)
