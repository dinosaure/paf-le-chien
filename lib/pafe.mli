...
val server : 'conn runtime -> 'conn -> Mimice.flow -> unit
...
val service :
  ('connected_flow -> (Mimice.flow * impl, 'error) result) ->
  ('flow -> ('connected_flow, 'error) result) ->
  ('t -> ('flow, ([> `Closed ] as 'error)) result) ->
  ('t -> unit) ->
  't service

val serve :
  't service -> 't -> [ `Initialized of (unit, [> `Closed ]) result Miou.t ]
