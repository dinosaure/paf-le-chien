  ...
  let get_certificate
    ...
    let `Initialized web_server, stop_web_server =
      ...
      let error_handler _dst ?request err _ =
        Logs.err (fun m ->
          m "error %a while processing request %a"
            pp_error err
    ...
    LE.provision_certificate ~production config ctx >>!= fun certificates ->
    Lwt_switch.turn_off stop_web_server >>= fun () ->
    web_server >>= fun () ->
    Lwt.return_ok certificates
end